...
{
int t,u,k;
float sfi,cfi,steta,cteta,galpha;
...
//glLineWidth(3);
  ...
  glMatrixMode (GL_PROJECTION);
  glLoadIdentity ();
//la gluPerspective vuole gli angoli in gradi
  galpha=deg(alpha);
//definisce matrice di proiezione
  gluPerspective(2*galpha, 1.0, 0.01, 100.0);
//printf("alpha= %f\n",2*galpha);
  ...
  glLoadIdentity();
//converte punto di vista da coordinate sferiche a cartesiane
  sfi=sin(fi);
  cfi=cos(fi);
  steta=sin(teta);
  cteta=cos(teta);
//definisce matrice di vista
  gluLookAt(D*sfi*cteta,D*cfi,D*sfi*steta,csx,csz,csy,0.0,1.0,0.0);
  ...
  for (k = 0;k <nedge;k++)
    {
        t=edge[k][0];
        u=edge[k][1];
        glVertex3f((GLfloat)x[t],(GLfloat)y[t],(GLfloat)z[t]);
        glVertex3f((GLfloat)x[u],(GLfloat)y[u],(GLfloat)z[u]);
    }
  glEnd();
